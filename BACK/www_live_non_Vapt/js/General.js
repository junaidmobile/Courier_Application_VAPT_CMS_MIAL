...
function LoadNavBar() {
    ...
    if (window.location.pathname.split("/").pop() == "EXP_AirsideRelease.html") {
    ...
    else if (window.location.pathname.split("/").pop() == "IMP_Segregation_Save.html") {
        $('#navhdrName').html("Segregation");
    }
        ...
        $('#navhdrName').html("Re-Print Package ID");
    }
...
function HHTLogout() {
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAImportFlightserviceURL + "LogOutUserSession",
            data: JSON.stringify({
                'UserID': UserId,
                'SessID': deviceUUID,
                'Status': '',
